...
- 推理5：设单元格集合 C 是两个互斥组 A 和 B 的交集，如果 A-C 内全部单元格排除 n ，那么 B-C 内全部单元格排除 n。
  ...
  称标记 C 的单元格是一个“核”，任意一宫内的任意一行、列都可以作为一个核，形成一对互相排除的区域。
...
这是一项工程技巧，相较于使用全局扫描，使用高效率"触发式"推理法，一个变量发生改变才去检测它可影响的推理，可以避免重复扫描没有变化的条件。
